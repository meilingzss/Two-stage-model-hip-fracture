...
UK <- draxicd[, -1]
# Create a new column 'grip' as the average of leftgrip and rightgrip
# Replace "Prefer not to answer" with "Do not know"
UK <- UK[!(UK$IADdifficulty %in% c("Do not know", "Prefer not to answer")), ]
# Keep only rows where walk is NOT "None of the above"
# Remove unwanted walk levels in one step
UK <- UK[!(UK$walk %in% c("None of the above", "Prefer not to answer")), ]
...
keep_cols <- c(
 "height", "walk", "smoke" , "grip", "IADdifficulty",
"weight", "age", "hip" , "SLDFX", "WRSTFX" , "neckbmd_Tscore",  "totalbmd_Tscore", "spinebmd_Tscore"
)
...
# Columns to convert
cols_to_factor <- c("smoke", "hip", "SLDFX", "WRSTFX", "IADdifficulty")

# Convert to factor
UK[cols_to_factor] <- lapply(UK[cols_to_factor], factor)
# Check structure
str(UK)
